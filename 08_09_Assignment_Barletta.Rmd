---
title: "08_09_Assignment_Barletta"
...
library(forcats)
library(readr)
training <- read_csv("Documents/8200/Assignments/A89 Data Files/training.csv")
View(training)
...
#first let's see HOW MANY total cars in the dataset are lemons
```{r}
table(training$isbadbuy)
```

#now let's see what PROPORTION of cars in the dataset  are lemons 
```{r}
...
```{r}
confusionMatrix(za_train$got_pizza,za_train$pred_lm_out)
```
...
```{r}
confusionMatrix(data=za_train$pred_logit_out,reference=za_train$got_pizza)
```
